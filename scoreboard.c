...
void load_register(int dest, int offset, int address)
{
    int counter = 0;
    if( offset + address <= 18)
    {
	float_reg[dest] = vals_in_mem[offset + address];
    }
    else if(offset + address > 18)
    {
	counter = offset+address -18;
	float_reg[dest] = vals_in_mem[counter - 1];
    }
}
...
int calculate( scoreboard_t instructions)
{
	...
	load_register(atoi(dest), atoi(offset), atoi(address));
	return 0;
    }
	...
	char dest[2];
        ...
        token = strtok(NULL, ")");
        strcpy(address, token);
        store_register(float_reg[atoi(dest)], atoi(offset), atoi(address));
        return 0;
    }
...
int scoreboarding(scoreboard_t instructions[], char used_reg[bufsize][5],
